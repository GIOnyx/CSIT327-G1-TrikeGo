"""Create the missing booking_ratingandfeedback table at the database level.

This migration contains only database operations because the model state
was already recorded in migration 0010_ratingandfeedback using
SeparateDatabaseAndState(state_operations=CreateModel, database_operations=[]).

We now run the database CreateModel operation so the actual table exists.
"""
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('booking', '0010_ratingandfeedback'),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.SeparateDatabaseAndState(
            # No state changes (model already recorded in 0010)
            state_operations=[],

            # Create the table in the database now
            database_operations=[
